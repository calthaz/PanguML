...
import zym.tensorflow.inception5h.LabelImage;
import zym.tensorflow.tools.LabelGenerator;
import zym.tensorflow.tools.NativeUtils;
import zym.tensorflow.tools.TFUtils;

/**
 * <span class="en">Read a TensorFlow Graph that is saved by 
 * {@code graph.as_graph_def()} and {@code graph_util.convert_variables_to_constants()}
 * for making predictions.</span>
 * <span class="zh">读取用 {@code graph.as_graph_def()}和{@code graph_util.convert_variables_to_constants()}保存的TensorFlow图
 * 用来出预测。</span>
 *
 */
public class GraphDriver implements Closeable{
	...
	/**
	 * <span class="en"> construct a GraphDriver with an active session and a graph</span>
	 * <span class="zh">构建一个有graph和活动session的GraphDriver</span>
	 * @param modelPath
	...
	/**
	 * <span class="en"> construct a GraphDriver with an active session and a graph<br>
	 * actually just a GraphDriver with {@code BATCH_SIZE = 1}</span>
	 * <span class="zh">构建一个有graph和活动session的GraphDriver<br>
	 * 其实就是 {@code BATCH_SIZE = 1}的GraphDriver</span>
	 * @param modelPath
	 * @param inputName op_name in graph
	 * @param outputName op_name in graph
	 * @param labelPath
	...
	/**
	 * <span class="en">predict the category of a single image, no score appended</span>
	 * <span class="zh">预测单个图像的类别，不附加分数</span>
	...
	/**
	 *
	 * <span class="en"> predict the category of a single image, with score appended in format: label(%.2f)</span>
	 * <span class="zh">预测单个图像的类别，附加分数格式为: 标签(%.2f)</span>
	 * @param imagePath string path to the image
	 * @return <span class="zh">文本标签</span><span class="en">text label</span>
	 */
	...
	/**
	 * 
	 * <span class="en">predict the category of a list of images, no score appended</span>
	 * <span class="zh">预测一个列表图片的类别，不带分数</span>
	...
	/**
	 * 
	 * <span class="en">Releases resources <br>
	 * i.e. graph and session</span>
	 * <span class="zh">释放资源，即session和graph</span>
	 */
	public void close(){
		sess.close();
		g.close();
  ...
  /**
   * 
   * <span class="en">process the image and return a float array with rgb color components </span>
   * <span class="zh">处理图片，返回带有rgb颜色分量的浮点数组</span>
   * @param img
   * @param normMethod 
   ...
   */
	private float[][][] toMatrix(BufferedImage img, String normMethod){
	  ...
	  /**
	   * 
	   * <span class="en">reads an image from file， process it and returns a float array with rgb color components </span>
	   * <span class="zh">从文件读取图像处理后返回带有rgb颜色分量的浮点数组</span>
	   * @param path
	   * @param normMethod    
	   * <span class="zh">用来把图片处理成一样大小的方法</span>
       * <span class="en">use normMethod to get images of the same size</span>
