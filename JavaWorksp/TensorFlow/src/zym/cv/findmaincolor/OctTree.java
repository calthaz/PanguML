...
/**
 * <span class="en">Use an Octree to extract main colors. See the links below.</span>
 * <span class="zh">用八叉树提取主要颜色，参见下面的链接</span>
 ...
 * <span class="en">This file is translated from JS and C++ at this repo:</span>
 * <span class="zh">这个文件是从下面这个仓库的JS和C++文件改写成java的</span>
...
public class OctTree implements ColorExtractor{
	...
	/**
	 * <span class="en">initialize the seven elements in {@code this.reducible} as null</span>
	 * <span class="zh">把 {@code this.reducible}中的七个元素初始化为null</span>
	 */
	public OctTree(){
		for(int i = 0; i < 7; i++) {
			//reducible.add(null);	
	...
	/**
	 * <span class="en">if maxColors is less than the number of nodes in the first layer, 
	 ...
	 * <span class="zh">如果需要的颜色小于第一层节点数（8）会报错，所以强制设为12以上。
	 * 返回前截取需要个数的颜色，如果没有足够颜色数组用{-1, -1, -1}填充</span>
	 * 
	 * @param img
	...
	/**
	 * 
	 * <span class="en">perform the oct-tree algorithm and compared it with a palette</span>
	 * <span class="zh">进行八叉树运算并和已有色板比较</span>
	...
	/**
	 * 
	 * <span class="en">Find the first key in a map with the given value 
	 * and remove the entry</span>
	 * <span class="zh">找到map中第一个有给定值的key，然后删除这个键值对</span>
	 * @param map
	...
	/**
	 * <span class="en">createNode
	 * if level<7 put it in the reducible[level] 
	 * and link node.next to what was there previously </span>
	 * <span class="zh">创建新节点，如果level<7 则放入reducible链表并且连上之前在那个位置的节点</span>
	...
	/**
	 * <span class="en">trim the leaves<br>
	 * adds leaves together and change their parent into a leaf node</span>
	 * <span class="zh">合并叶子，并把父节点设为叶子节点</span>
	 * 
	 */
	...
	/**
	 * <span class="en">reads the colors in the leaves by {@code leaf.color/leaf.pixelCount}</span>
	 * <span class="zh">读取树上的颜色，按照{@code leaf.color/leaf.pixelCount}计算</span>
	 * <b>Note:recursive</b>
