...
class FreezeGraphTest(test_util.TensorFlowTestCase):
  ...
  def _testFreezeGraph(self, saver_write_version):
    ...
    checkpoint_prefix = os.path.join("./test_temp_dir", "saved_checkpoint")
    ...
    with ops.Graph().as_default():
      ...
      graph_io.write_graph(sess.graph, "./test_temp_dir", input_graph_name)

    # We save out the graph to disk, and then call the const conversion
    # routine.
    input_graph_path = os.path.join("./test_temp_dir", input_graph_name)
    input_saver_def_path = ""
    input_binary = False
    output_node_names = "output_node"
    restore_op_name = "save/restore_all"
    filename_tensor_name = "save/Const:0"
    output_graph_path = os.path.join("./test_temp_dir", output_graph_name)
    clear_devices = False
