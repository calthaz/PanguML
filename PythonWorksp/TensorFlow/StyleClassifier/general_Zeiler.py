...
INITIAL_LEARNING_RATE = 0.001       # Initial learning rate. 0.1
'''
decayed_learning_rate = INITIAL_LEARNING_RATE *
                        LEARNING_RATE_DECAY_FACTOR ^ (global_step / 
                        (NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN / FLAGS.batch_size * NUM_EPOCHS_PER_DECAY))
'''
...
def inference(images):
  ...
  with tf.variable_scope('conv1') as scope:
    ...
    biases = _variable_on_cpu('biases', [96], tf.constant_initializer(0.0))# why zeros?0.0
  ...
  with tf.variable_scope('conv2') as scope:
    ...
    biases = _variable_on_cpu('biases', [256], tf.constant_initializer(0.000000001))
    pre_activation = tf.nn.bias_add(conv, biases)
  ...
  with tf.variable_scope('conv3') as scope:
    ...
    biases = _variable_on_cpu('biases', [384], tf.constant_initializer(0.00))
    pre_activation = tf.nn.bias_add(conv, biases)
  ...
  with tf.variable_scope('conv5') as scope:
    ...
    biases = _variable_on_cpu('biases', [256], tf.constant_initializer(0.0000))
    pre_activation = tf.nn.bias_add(conv, biases)
  ...
  with tf.variable_scope('local7') as scope:
    weights = _variable_with_weight_decay('weights', shape=[4096, 2048],#Zeiler uses 4096 again
                                          stddev=0.04, wd=0.004)
  ...
  with tf.variable_scope('softmax_linear') as scope:
    weights = _variable_with_weight_decay('weights', [2048, NUM_CLASSES],
                                          stddev=1/192.0, wd=0.0)#why 1/192? anything to do with weights?
