...
def inference(images):
  ...
  # this will display random 3 filters from the 64 in conv1
      ...
      conv_trans = tf.nn.conv2d_transpose(conv1, kernel, output_shape=[FLAGS.batch_size, 32, 32, 3], strides=[1,1,1,1], padding='SAME')
      tf.summary.image("conv2d_transpose_layer1", conv_trans)
      with tf.name_scope("find_max_activ"):
        #find strongest activation here?
        #what is a strongest activation? contrast?
        #how? anyway let's plays
        flattened_conv1 = tf.reshape(conv1, [FLAGS.batch_size, 32*32, 64])#[1,1024,64]
        ...
        max_slice = tf.slice(conv1, [0,0,0,max_index], [FLAGS.batch_size,32,32,1])
        ...
        max_trans = tf.nn.conv2d_transpose(max_filters, kernel, output_shape=[FLAGS.batch_size, 32, 32, 3], strides=[1,1,1,1], padding='SAME')
        tf.summary.image("max_transpose_layer1", max_trans, max_outputs=16)
  ...
  with tf.name_scope('visualization'):
    ...
    unpooled = tf.image.resize_images(pool1, [32, 32])
    pool1_trans = tf.nn.conv2d_transpose(unpooled, kernel, output_shape=[FLAGS.batch_size, 32, 32, 3], strides=[1,1,1,1], padding='SAME')
    tf.summary.image("pool1_transpose_layer1", pool1_trans, max_outputs=16)
