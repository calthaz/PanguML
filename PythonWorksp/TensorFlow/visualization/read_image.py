...
def _generate_image_and_label_batch(image, label, min_queue_examples,
	...
	tf.summary.image('images', tf.reshape(images, [batch_size, IMAGE_SIZE, IMAGE_SIZE,3]))
...
def read_shuffled_labeled_image_list(image_list_file, batch_size=-1):
	"""Reads a .txt file containing pathes and labeles
	Args:
	   image_list_file: a .txt file with one /path/to/image per line
	   label: optionally, if set label will be pasted after each line
...
def static_resize_inputs(eval_data, batch_size):
	...
	Returns:
	images: Images. 2D array of [batch_size, IMAGE_SIZE*IMAGE_SIZE*3] size.
