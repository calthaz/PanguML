#http://blog.csdn.net/superman_xxx/article/details/63686529
# coding:UTF-8
...
def variable_with_weight_loss(shape, stddev, wl, name="w"):
...
image_holder = tf.placeholder(tf.float32, [batch_size, 24, 24, 3], name='image-holder')
label_holder = tf.placeholder(tf.int32, [batch_size], name='label-holder')
...
with tf.name_scope('conv1'):
	...
	bias1 = tf.Variable(tf.constant(0.0, shape = [64]), name = 'b')
	# 使用激活函数进行非线性化
...
with tf.name_scope('conv2'):
	...
	# bias值全部初始化为0.1。
	bias2 = tf.Variable(tf.constant(0.1, shape = [64]), name='b')
...
with tf.name_scope('fc1'):
	...
	# bias值初始化为0.1
	bias3 = tf.Variable(tf.constant(0.1, shape = [384]), name='b')
	# 使用激活函数进行非线性化
...
with tf.name_scope('fc2'):
	...
	bias4 = tf.Variable(tf.constant(0.1, shape = [192]), name='b')
...
with tf.name_scope('fc_output'):
	...
	# 模型Inference的输出结果
	logits = tf.nn.relu(tf.matmul(local4, weight5) + bias5, name="output-tensor")
...
writer = tf.summary.FileWriter('/tmp/tensorflow/cifar10/logs/csdn1')
