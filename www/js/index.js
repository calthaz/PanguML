...
$(function(){
	...
	myDropzone.on("success", function(file, response) {
  	...
  	$("button.start-infer.bed").click(function(){
  		...
  		$.post("uploadHandler.php", {action:"start", bed:true}).done(function(data){
  			//console.log(data);
            try{
  	...
  	$("button.start-infer.inception").click(function(){
  		...
  		$btn.text("...");
  		$btn.css("pointer-events", "none");
  		$.post("uploadHandler.php", {action:"start", inception:true}).done(function(data){
  			//console.log(data);
            try{
                data = JSON.parse(data);
                for(var x in data.results){
  	...
  	$("main").on("click", "a.infer-pic.bed", function(event){
  		var filename = $(event.target).parents("div.img-wrapper").find("img").attr("src");
  		//console.log(filename);
  		$.post("uploadHandler.php", {infer:filename, bed:true}).done(function(data){
  			try{
                data = JSON.parse(data);
                for(var x in data.results){
                	var filename = x.replace(/\\/g, "/");
  	...
  	$("main").on("click", "a.infer-pic.inception", function(event){
  		var filename = $(event.target).parents("div.img-wrapper").find("img").attr("src");
  		//console.log(filename);
  		$.post("uploadHandler.php", {infer:filename, inception:true}).done(function(data){
  			try{
                data = JSON.parse(data);
                for(var x in data.results){
                	var filename = x.replace(/\\/g, "/");
