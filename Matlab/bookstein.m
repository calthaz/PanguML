%this file is taken from Serge Belongie's Shape Context code, although I do
%not know who the original author is.
...
K=r2.*log(r2+eye(N,N) + eps); % add identity matrix to make K zero on the diagonal
P=[ones(N,1) X];
L=[K  P
   P' zeros(3,3)];
V=[Y' zeros(2,3)];
