...
unsigned int ThreadProc( void* lpParameter);
...
//	CMatlabToOpenCV::release_iplimage(iplB);
	...
	{
       hThread[i]=(HANDLE)_beginthreadex(0,0, &ThreadProc,&param[i], 0x00000004,&threadID[i]);
...
unsigned int   ThreadProc( void* lpParam)
{
